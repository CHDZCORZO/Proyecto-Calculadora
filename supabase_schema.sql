...
CREATE TABLE public.archivos_cargados (
  ...
  marca TEXT,
  tramite TEXT,
  convenio TEXT DEFAULT 'IMSS PENSIONADOS',
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Tabla: tablas_cotizador
CREATE TABLE public.tablas_cotizador (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  id_oferta TEXT,
  plazo INTEGER,
  monto NUMERIC,
  ...
  tramite TEXT,
  convenio TEXT DEFAULT 'IMSS PENSIONADOS',
  archivo_id BIGINT REFERENCES public.archivos_cargados(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
